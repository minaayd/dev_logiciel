...
def train_model(X: pd.DataFrame, y: pd.Series) -> RandomForestClassifier:
    ...
    try:
        ...
        model = RandomForestClassifier(n_estimators=100, max_depth=5, random_state=1)  # Initialisation du modèle
...
def load_preprocessed_data() -> tuple[pd.DataFrame, pd.Series, pd.DataFrame, pd.DataFrame]:
    ...
    if os.path.exists(preprocessed_train_features_path) and os.path.exists(preprocessed_train_labels_path) and os.path.exists(preprocessed_test_features_path):
        try:
            X = pd.read_csv(preprocessed_train_features_path)  # Chargement des caractéristiques d'entraînement
            y = pd.read_csv(preprocessed_train_labels_path)  # Chargement des étiquettes d'entraînement
            X_test = pd.read_csv(preprocessed_test_features_path)  # Chargement des caractéristiques de test
            _, test_data = load_data(train_data_path, test_data_path)  # Chargement des données brutes de test
